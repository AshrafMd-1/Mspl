links = {
    ...
    // https://app.powerbi.com/groups/me/reports/b4a53daa-a519-44ca-8b10-bcb1b8e7cfcf/ReportSectionfbdc3e3fdaae956b9024?filter=RM_x0020_Price/Plant eq 'Bhurwal'
};
...
function giveFilters(filterValues) {
    ...
    for (filterValue of filterValues) {
        for (filter of filters) {
            if (filter.includes(filterValue.slice(0, 1).toUpperCase() + filterValue.slice(1).toLowerCase())) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request && request.text && request.text.includes("filter")) {
        ...
        if (newLink) {
            ...
            link = `Opening ${request.text.toLowerCase().split("filter")[0].split("with")[0].trim()} with filter ${filter}`;
        } else {
